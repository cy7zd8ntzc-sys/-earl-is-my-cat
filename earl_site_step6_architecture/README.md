...
- **HTML:** static pages
- **CSS:** shared static stylesheets in `assets/css/`
- **JavaScript:** small vanilla JS interactions in `assets/js/`
- **Fonts:** Google Fonts
- **Images:** extracted static images in `assets/images/`
- **Backend:** none yet
...
### Step 3 intake and provider-review update

This bundle adds a more realistic visit-intake prototype and one-page provider dashboard mockup:

- `visit.html` now uses a six-step minimal intake: concern/HPI, photos, medical snapshot, safety check, identity/state, and consent/payment/submit.
- `intake-config.json` documents the current workflow, photo policy, dispositions, and urgent safety flags.
- `provider.html` is the primary all-in-one review screen for provider decision-making and consult closure; `provider-dashboard.html` redirects there.
- `docs/VISIT_INTAKE_ARCHITECTURE.md` documents the intended production workflow.
- `docs/PROVIDER_DASHBOARD_SPEC.md` documents the provider dashboard concept.

The photo workflow uses a three-photo operational target, not a universal legal requirement. The production workflow still needs state-specific legal review, secure storage, audit logging, payment integration, and pharmacy integration.
...
| `visit.html` | Prototype online visit intake flow with minimal HPI, photo upload, safety gates, consent, and payment placeholder |
| `provider.html` | One-page provider review and consult-closure prototype |
...
- Payment processor integration and payment/refund policy.
- Secure pharmacy handoff and compounding-pharmacy workflow.
- Provider dashboard audit logging and consult-close events.
...
This repo is organized as a static prototype with shared assets:

- `assets/css/` — shared/page-level stylesheets.
- `assets/js/` — shared and page-specific JavaScript.
- `assets/images/` — extracted image assets.
- `partials/` — canonical header/footer markup for future template use.
- `templates/` — clone-ready page templates.
- `docs/` — compliance and refactor notes.

Open `index.html` locally to preview the prototype. No build step is required.

## Step 3 intake architecture

The prototype now includes a more production-oriented visit intake and one-page provider dashboard preview:

- `visit.html` - minimal patient intake with concern selection, photo upload, medical snapshot, safety flags, consent, and payment placeholder.
- `provider.html` - one-page provider review and closeout prototype.
- `docs/VISIT_INTAKE_ARCHITECTURE.md` - workflow and data model.
- `intake-config.json` - machine-readable workflow notes.

This is still a static prototype. Do not use it for real PHI until secure authentication, encrypted storage, consent logging, payment processing, audit trails, and pharmacy transmission are implemented.


## Step 4: internal formulary prototype

This version adds a clinician-only formulary quick-pick layer to `provider.html`. The formulary is stored in `assets/data/formulary.json` and mirrored into `assets/js/formulary-data.js` so the static demo can run without a backend. The templates are decision-support drafts only; the treating clinician must verify diagnosis, contraindications, final prescription language, and pharmacy routing before sending anything in production.


## Adult-only launch scope

This prototype is currently configured for patients 18 and older only. Pediatric/guardian-consent workflows are intentionally out of scope for launch. Do not collect photos or medical information for minors in this version. Eczema and molluscum are held from the active condition list for now.

## Step 6: Production architecture plan

Production architecture planning has been added under:
...
These files define the recommended PHI-safe backend, adult-only patient intake, encrypted photo storage, provider one-page dashboard data model, Stripe payment sequence with no PHI metadata, and secure compounding pharmacy handoff.
