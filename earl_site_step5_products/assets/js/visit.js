...
var CONCERNS=[
  ...
  ["seb-derm","Dandruff / seborrheic dermatitis"],["sweating","Excessive sweating"],["warts","Warts"],["herpes","Cold sores & herpes"],
...
var RED_FLAGS={
  ...
  "warts":["Eye involvement, severe pain, rapid change/bleeding, diabetes/poor circulation, or immunosuppression"],
...
function ageFromDob(v){if(!v)return null;var d=new Date(v+'T00:00:00');if(isNaN(d.getTime()))return null;var now=new Date();var y=now.getFullYear()-d.getFullYear();var m=now.getMonth()-d.getMonth();if(m<0||(m===0&&now.getDate()<d.getDate()))y--;return y;}
function isAdultDob(){var a=ageFromDob(fieldValue('dob'));return a!==null&&a>=18;}
...
function valid(){
  ...
  if(step==='concern'){
    var adultOk=!!($('ageConfirm')&&$('ageConfirm').checked); showError('ageConfirm', !adultOk);
    var okConcern=!!selectedConcern(); showError('concern', !okConcern);
    return adultOk && okConcern && requireField('chiefConcern','chiefConcern') && requireField('duration','duration');
  }
  if(step==='photos'){
  ...
  if(step==='identity'){
    ...
    var adultDobOk=ok&&isAdultDob();
    var dg=$('dobGate'); if(dg) dg.classList.toggle('show', ok&&!adultDobOk);
    showError('identity', !ok||!adultDobOk);
    return ok&&adultDobOk;
  }
  if(step==='consent-payment'){
    var ok=['consentAdult','consentTelehealth','consentRx','consentCash','consentPrivacy'].every(function(id){return $(id).checked;});
...
function buildSubmission(){
  ...
  return {
    ...
    ageConfirmed:!!($('ageConfirm')&&$('ageConfirm').checked),
    ageAtSubmission:ageFromDob(fieldValue('dob')),
    redFlags:flags,
...
function buildReview(){
  ...
  var rows=[
    ...
    ['Photos',s.photoCount+' added'+(s.photoOverride?' · override acknowledged':'')],['Medical snapshot',s.medicalSnapshot],['Pregnancy/breastfeeding',s.pregnancy],['Adult eligibility',s.ageConfirmed?'18+ confirmed':'Not confirmed'],['Patient',((s.first+' '+s.last).trim()||'—')],['DOB',s.dob||'—'],['State',s.state||'—'],['Safety gate',s.redFlags.length?s.redFlags.length+' item(s) selected':'None selected']
  ];
