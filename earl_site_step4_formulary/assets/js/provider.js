...
function normalizeKey(s){return String(s||'').toLowerCase().replace(/&/g,'and').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
...
var visit=loadVisit();
var formulary=window.EARL_FORMULARY||{conditions:{},routing:{},globalRules:[]};
...
function conditionKeys(){return Object.keys(formulary.conditions||{}).sort(function(a,b){return formulary.conditions[a].label.localeCompare(formulary.conditions[b].label);});}
function bestConditionKey(){
  var wanted=normalizeKey(visit.concern||'');
  var aliases={
    'rosacea-redness':'rosacea','redness':'rosacea','dark-spots-hyperpigmentation':'dark-spots','hyperpigmentation':'dark-spots','fine-lines-aging':'aging','anti-aging':'aging','hair-loss':'hairloss','seborrheic-dermatitis':'seb-derm','dandruff-seb-derm':'seb-derm','excessive-sweating':'sweating','warts-molluscum':'warts','aphthous-ulcers-canker-sores':'aphthous-ulcers','canker-sores':'aphthous-ulcers','folliculitis-ingrown-hairs':'folliculitis','intertrigo-yeast-rash':'intertrigo','nail-fungus':'nail-fungus','genital-warts':'genital-warts','perioral-dermatitis':'perioral-dermatitis','keratosis-pilaris':'keratosis-pilaris'
  };
  if((formulary.conditions||{})[wanted])return wanted;
  if(aliases[wanted])return aliases[wanted];
  var found=conditionKeys().find(function(k){var label=normalizeKey(formulary.conditions[k].label);return label===wanted||label.indexOf(wanted)>=0||wanted.indexOf(label)>=0;});
  return found||conditionKeys()[0]||'';
}
function templateText(t){
  if(!t)return '';
  return ['Template: '+t.title,'Route: '+t.route,'Diagnosis: '+t.diagnosis,'Plan: '+t.plan,'Rx: '+t.rx,'Counseling: '+t.counseling,'Avoid if: '+(t.avoidIf||[]).join('; '),'Follow-up: '+(t.followUp||'per clinician')].join('\n');
}
function renderTemplateDetail(t){
  var box=$('templateDetail');
  if(!t){box.innerHTML='<p class="soft">Choose a template to preview diagnosis, plan, Rx handoff, counseling, and safety exclusions.</p>';return;}
  box.innerHTML='<h3>'+esc(t.title)+'</h3>'+
    '<dl class="template-facts">'+
    '<dt>Route</dt><dd>'+esc(t.route)+'</dd>'+
    '<dt>Diagnosis</dt><dd>'+esc(t.diagnosis)+'</dd>'+
    '<dt>Plan</dt><dd>'+esc(t.plan)+'</dd>'+
    '<dt>Rx handoff</dt><dd>'+esc(t.rx)+'</dd>'+
    '<dt>Counseling</dt><dd>'+esc(t.counseling)+'</dd>'+
    '<dt>Avoid / pause</dt><dd>'+esc((t.avoidIf||[]).join('; ')||'None listed')+'</dd>'+
    '<dt>Follow-up</dt><dd>'+esc(t.followUp||'Per clinician')+'</dd>'+
    '</dl>';
  $('safetyTags').innerHTML=(t.tags||[]).map(function(tag){return '<span>'+esc(tag)+'</span>';}).join('');
}
function renderFormulary(key){
  var condition=(formulary.conditions||{})[key];
  var list=$('rxTemplateList'), supportive=$('supportiveList'), route=$('routingNote');
  selectedTemplate=null;
  $('applyTemplate').disabled=true;
  $('safetyTags').innerHTML='';
  if(!condition){list.innerHTML='<p class="soft">No formulary pathway found.</p>';supportive.innerHTML='';renderTemplateDetail(null);return;}
  route.textContent='Default follow-up: '+(condition.defaultFollowUp||'per clinician')+'. Review safety screen before any prescription.';
  list.innerHTML=(condition.rxTemplates||[]).map(function(t,idx){return '<button type="button" class="rx-template" data-idx="'+idx+'"><strong>'+esc(t.title)+'</strong><span>'+esc(t.route)+'</span></button>';}).join('')||'<p class="soft">No templates configured.</p>';
  supportive.innerHTML=(condition.supportiveCare||[]).map(function(item){return '<li>'+esc(item)+'</li>';}).join('')||'<li>None configured.</li>';
  renderTemplateDetail(null);
  Array.prototype.forEach.call(list.querySelectorAll('[data-idx]'),function(btn){btn.addEventListener('click',function(){
    Array.prototype.forEach.call(list.querySelectorAll('.rx-template'),function(b){b.classList.remove('active');});
    btn.classList.add('active');
    selectedTemplate=condition.rxTemplates[Number(btn.getAttribute('data-idx'))];
    $('applyTemplate').disabled=false;
    renderTemplateDetail(selectedTemplate);
  });});
}
function renderFormularySelectors(){
  if(!$('formularyConcern'))return;
  $('formularyVersion').textContent=(formulary.version||'prototype').replace('2026-06-','');
  var select=$('formularyConcern');
  select.innerHTML=conditionKeys().map(function(k){return '<option value="'+esc(k)+'">'+esc(formulary.conditions[k].label)+'</option>';}).join('');
  select.value=bestConditionKey();
  renderFormulary(select.value);
  select.addEventListener('change',function(){renderFormulary(select.value);});
  $('applyTemplate').addEventListener('click',function(){
    if(!selectedTemplate)return;
    $('dx').value=selectedTemplate.diagnosis||'';
    $('plan').value=selectedTemplate.plan||'';
    $('rxText').value=selectedTemplate.rx||'';
    $('patientMessage').value='I reviewed your intake and photos. Working diagnosis: '+(selectedTemplate.diagnosis||'skin concern')+'. '+(selectedTemplate.counseling||'Please follow the plan and reach out if symptoms worsen.');
    var route=selectedTemplate.route||'';
    if(route.indexOf('compounded')>=0){$('rxAction').value='Send compounded prescription';}
    else if(route.indexOf('commercial')>=0){$('rxAction').value='Send commercial pharmacy prescription';}
    else if(route.indexOf('otc')>=0||route.indexOf('white-label')>=0||route==='no-rx'){$('rxAction').value='OTC/supportive care only';}
    $('decision').value='Ready to treat';
    $('result').textContent='Template applied. Edit before sending.';
  });
}
...
renderTop();renderFacts();renderFlags();renderPhotos();renderFormularySelectors();setTemplates();wireActions();
})();
